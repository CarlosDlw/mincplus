...
[[nodiscard]] CheckRun run(const std::vector<std::string>& inputs, bool showAst = false,
                           bool showTypes = false, sema::Target target = sema::kDefaultTarget,
...
TEST(CheckCommandTest, ACleanUnitSucceedsAndPrintsTheTypeTable) {
  ...
  // The vocabulary the summary is written in, printed once for the invocation.
  ...
  EXPECT_NE(result.out.find("0 error(s)"), std::string::npos);
}

TEST(CheckCommandTest, AnErrorGoesToStderrAndFailsTheCommand) {
  ...
  // Diagnostics go to stderr; the summary stays on stdout, because a reader
  // wants to know what was built before knowing it was wrong.
  EXPECT_NE(result.out.find("1 error(s)"), std::string::npos);
  EXPECT_EQ(result.out.find("error["), std::string::npos);
}
...
TEST(CheckCommandTest, ConversionIsAWarningAndKeepsTheExitCode) {
  ...
  const CheckRun with = run({loud.path()}, /*showAst=*/false, /*showTypes=*/false,
                            sema::kDefaultTarget, /*warnConversion=*/true);
...
TEST(CheckCommandTest, EveryExampleChecks) {
  ...
  const CheckRun result = run(files);
  ...
  EXPECT_GE(summaries, files.size());
}
