...
| 14 | `...` in the lexer | **One token, `Ellipsis`.** It used to be three `Dot`s: the language had no variadic syntax, so classifying `...` would have put a C *macro* concept in the language's lexer, and the preprocessor rebuilt the marker by checking that three dots were adjacent. A function declaration can be variadic now (`extern fn i32 printf(fmt: str, ...);`), so the marker is the language's too, and the reconstruction is gone — one rule, in the lexer, instead of two that agreed by luck. Longest match as everywhere: `....` is `...` then `.`, and a *spaced* `. . .` is three tokens, which was never an ellipsis to either reader. A leading dot is still a number (`.5`), so the marker cannot be confused with one. |
| 15 | `.` in front of a digit is a fraction **unless the character before it is another `.`** | `a[1..2]` and `.5` are both legal spellings, and longest-match cannot tell them apart: `.` + digit is `.2` in the one and part of `..` in the other. The rule is one character of lookbehind — the only place in this lexer where a token's reading depends on the byte *before* it — and it was written when the range operator a slice will use (`..`, `arrays.md` step 10) turned out to be unlexable. Without it the reserved spelling exists in the grammar and in no input, which is worse than not reserving it: `a[1..2]` lexed as `1`, `.`, `.2` and the parser answered "expected `]`" about a bracket two tokens past the operator the reader typed. |
