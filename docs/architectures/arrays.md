...
| **28** | **The frame is bounded too**: an object this function would place in its own frame may not exceed `kMaxStackObjectBytes` (16 MiB), refused at the declaration, at the three places a slot is created (a local binding, a by-value argument's caller-side copy, and the copy of an rvalue aggregate a subscript steps into -- decision 32) | Decision 14 promised this and the promise is worth restating where it is enforced: without it `let a: [1 << 40]u8;` builds a terabyte `alloca`, the backend emits a stack subtraction that size, and the failure is a segfault at the function's first instruction — a bug handed to a debugger instead of to a diagnostic. The by-value *parameter* has no slot of its own (13 says its storage is the pointer it arrived as), which is why the *caller's* copy is the second call site and not the callee's binding |

| **30** | **A type written as a word in front of braces is the same literal**: `Row{1, 2, 3}` is `TypedInitializer`, the type read by the type reader, and the *brace* is what says the word was a type. One word and its optional `<...>` list, then `{` | The bracket form is not the only way a reader can name a type, and the alternative -- brace-suffixed values only for the spellings the compiler writes itself -- would make an alias a second-class way to say a type. Nothing is looked up: the shape is decided by tokens, so an alias declared in another file (and, one day, imported) reads the same as a local one. It also fixes the reading of `Row{1, 2, 3}[0]`: a *value* is element-readable whether or not a binding named it |
| **31** | **In a condition, a word in front of a brace is the body of the statement** -- unless the text says otherwise. Two facts say otherwise, and both are lexical: a `<...>` **list** (a comparison has none, so `Vec<i32>{...}` is a type there) and a **comma at the top level of the braces** (no block of this grammar can hold one: every statement ends with `;` or `}`, so the group is a value). The read is on everywhere else, and *inside a group* it is lifted, because `)` and `]` bound the expression (`if (Row{1, 2, 3}[0] > 0) { }`) | `if x { }` and `if Row{1, 2, 3}[0] > 0 { }` are the same tokens up to the brace and the first is the common case, so *something* has to give -- and the language refuses to decide it by looking a name up, which is what forces Rust's `NO_STRUCT_LITERAL` to be a restriction plus name resolution. Two lexical facts are enough here, and each can only be present in text that is not a legal block, so the sentence can fire on a program that was already broken and never on one that was not. The bare form in a condition is the one spelling that needs the parentheses, and the report says so by name (`InitializerInCondition`) |
| **32** | **An rvalue aggregate gets a frame slot when something has to step into it.** `Row{1, 2, 3}[0]`, `f()[i]`: the value is copied into an entry-block slot and the element is the copy's | The checker accepts a subscript of a temporary and the *lowering* has no object to address -- which is the one thing this stage may not do: refuse what the checker let through. The copy is the same one a by-value aggregate argument already makes (decision 28's third slot), so the rule and its bound are shared rather than re-stated |
...
// ...and the same literal with the type written as a **name** (decision 30)
type Row = [3]i32;
type Vec<T> = [4]T;
Row{1, 2, 3}          // the name in front of the braces, nothing else needed
Vec<i32>{1, 2, 3, 4}  // a generic name, with its own arguments written
Row{7, 8, 9}[2]       // a value like any other: the temporary is element-read
...
| `parse` | `scanTypeRun` and `parseType`/`parseTypeAndName` accept a bracketed count between the `*`s and the words; a primary expression may start with `[`, with the one-token rule above, and a **word** may open the same node (`initializerRead`, with `InitializerRestriction` and `bracesHoldTopLevelComma` for the condition -- decision 31). Two new syntax kinds (`ArrayLiteral`, `TypedInitializer`), a third error code (`InitializerInCondition`), and a `Type` node that keeps its brackets and count as children so the tree stays lossless |
