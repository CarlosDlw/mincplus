...
A substitution is `{Param(unit, owner, i) ↦ τ}` and `τ` may mention parameters of an
...
| 5 | **One new kind, `Param`, with identity `(unit, owner, index)` and the source spelling kept** | The store's answer to 'same type?' is structural, so a parameter needs an identity that is stable and cannot collide between two declarations. The spelling is kept because `spelling` is what a diagnostic prints and it has no other source. The **unit** was added after the fact and is the correction of a real defect: `owner` is a node id and a tree is numbered from zero *per file*, while the store is one per compilation — so the first binder read at node 5 answered for the binder written at node 5 of every other input, class included. Which file failed therefore depended on the order the inputs were named on the command line, and the CI's three runners disagreed. A declaration is a node **of a file**; the identity has to say which |
